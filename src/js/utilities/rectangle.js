...
Rect.prototype.getAxis = function () {
  return this._axis || (this._axis = [
    new Coord([1, 0]).rotateDeg(this.angle),
    new Coord([0, 1]).rotateDeg(this.angle),
  ]);
};
...
Rect.prototype.getMid = function () {
  return new Coord([this.x + this.width / 2, this.y + this.height / 2]);
};

Rect.prototype.getCorners = function () {
  if (this._corners) return this._corners;
  var TL = new Coord([this.x, this.y]);
  var TR = new Coord([this.x + this.width, this.y]);
  var BR = new Coord([this.x + this.width, this.y + this.height]);
  var BL = new Coord([this.x, this.y + this.height]);
...
Rect.prototype.collision = function (rect) {
  ...
  for (var i = 0; i < axis.length; i++) {
    ...
    if (thisProj.max - rectProj.min <= COLL_EPS || rectProj.max - thisProj.min <= COLL_EPS) {
      return false;
    }
  }
