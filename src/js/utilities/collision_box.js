...
function CollBox(parent, dimension, angle) {
  this.dim = new Coord(dimension);
  this.parent = parent;
...
CollBox.prototype.maxDim = function () {
  var axis1 = new Coord([this.dim.x, 0]);
  var axis2 = new Coord([0, this.dim.y]);
  axis1.plusAngleDeg(this.angle).makeAbs();
  axis2.plusAngleDeg(this.angle).makeAbs();
  return new Coord([Math.max(axis1.x, axis2.x), Math.max(axis1.y, axis2.y)]);
};
...
CollBox.prototype.newAxis = function (startPos, startAngle) {
  var newMe = new CollBox(this.parent, this.dim.dup(), this.angle - startAngle);
