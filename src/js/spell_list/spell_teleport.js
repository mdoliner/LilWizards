...
var TeleportSpell = Spell.extend({
  img: require('graphics/spell_teleport.png'),
  dim: [12, 12],
  sType: 'ray',
  sId: 'teleport',
  initialize: function () {
  ...
  tickEvent: function () {
    ...
    if (action === 'release' || action === 'none') {
      this.pos.min([Game.DIMX, Game.DIMY]);
      this.pos.max(0);
      var collisions;
      ...
      this.remove();
      var funct = function () {
        ParticleSplatter(3, TeleportParticleLine.bind(this, times));
        if (times <= 0) {
  ...
  removeEvent: function () {
    ParticleSplatter(10, TeleportBurstParticles.bind(this));
  },
});

SpellList.Teleport = function (spellIndex) {
...
export default SpellList.Teleport;
