...
var CrashSpell = Spell.extend({
  vel: [0, 15],
  img: require('graphics/spell_crash2.png'),
  dim: [10, 5],
  imgBaseAngle: 90,
  duration: -1,
  sType: 'melee',
  ...
  tickEvent: function () {
    if (this.impact) {
      ...
      if (this.caster.onGround) {
        ...
        ParticleSplatter(40, CrashParticles.bind(this));
      }
    }
  },
});
...
SpellList.Crash = function (spellIndex) {
  if (this.onGround) {
    ...
    ParticleSplatter(10, function () {
      var randVel = new Coord([-Math.random() * 4, 0]).plusAngleDeg(Math.floor(Math.random() * 2) * 180);
      return {
        pos: this.pos,
        vel: randVel,
        game: this.game,
...
export default SpellList.Crash;
